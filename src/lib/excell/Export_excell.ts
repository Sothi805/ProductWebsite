...
export const excelSheetTitle = (
  ...
  worksheet.getCell("B2").value = "MY SYSTEM API HANDLE BY @MONY096";
...
export const excelSheetHeader = (
  ...
  for (let index = 0; index < columns.length; index++) {
    ...
    worksheet.getCell(cell).fill = {
      type: "pattern",
      pattern: "solid",
      fgColor: { argb: "0284C7" }, // Header background color
      ...headers.style?.fill,
    };
...
export const exportDefaulExcelTemplate = async (sheet: Sheet) => {
  ...
  let worksheet:any = workbook.addWorksheet(sheetName ?? "Sheet", {
